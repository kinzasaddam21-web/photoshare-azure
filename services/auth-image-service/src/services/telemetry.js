// Must be required BEFORE other modules to capture all telemetry
...
if (config.appInsights.enabled) {
  try {
    ...
    client.context.tags[client.context.keys.cloudRole] = 'auth-image-service';
    console.log('[telemetry] Application Insights enabled');
  } catch (err) {
    console.warn('[telemetry] Failed to start App Insights:', err.message);
  }
} else {
  console.log('[telemetry] Application Insights disabled (no connection string)');
}
